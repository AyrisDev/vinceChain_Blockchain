			...
			Context("with a sender's claims record", func() {
				...
				Context("with completed actions", func() {
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						err := s.napathOsmosisVince.RelayPacket(CreatePacket("10000", "avce", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						...
						ibcVince := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, avceIbcdenom)
						Expect(ibcVince).To(Equal(sdk.NewCoin(avceIbcdenom, coinVince.Amount)))
